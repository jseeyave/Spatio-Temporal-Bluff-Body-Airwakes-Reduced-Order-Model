...
# DataLoader ###############################################################################################
class CFDDataLoader:
    def __init__(self, velocity_target_dir, blury_dir,fm_dir,cases, batch_size=4, num_timesteps=10000, shuffle=True, vel_shape=(176,80,64,3), FMshape=(176,80,64,1)):       
...
fm_dir = "/home/jseeyave3/FluidMasks/"
...
ACC_STEPS = 8
accum_counter = tf.Variable(0, trainable=False, dtype=tf.int64)
grad_buffer = [tf.Variable(tf.zeros_like(v), trainable=False)
               for v in diffusion.trainable_weights]
@tf.function()
